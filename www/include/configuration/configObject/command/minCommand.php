...
} elseif (isset($_POST["command_id2"]) && $_POST["command_id2"]) {
...
$query = "SELECT `command_id`, `command_name` FROM `command` WHERE `command_type` = '2' ORDER BY `command_name`";
$DBRESULT = $pearDB->query($query);
...
} elseif ($cmd["command_type"] == "2") {
    $form->addElement('header', 'information', _("Check command"));
    $elemname = "command_id2";
} elseif ($cmd["command_type"] == "3") {
